...
const Sprite = function(ctx, x, y) {
    ...
    const drawSprite = function() {
        ...
        const srcY = sequence.y;
    
        /* Draw the sprite correctly */
        ctx.drawImage(
            sheet,
            srcX, srcY, sequence.width, sequence.height, // Source rectangle
            parseInt(x - size.width / 2),  parseInt(y - size.height / 2), // Destination position adjusted to center
            size.width, size.height // Destination size
        );
    ...
    const update = function(time) {
        ...
        if(time - lastUpdate > sequence.timing){
            if(index==(sequence.count-1)&&sequence.loop==false){
                lastUpdate = time;
                drawSprite();
            }
            else{ 
